...
class LoopProcessor:
    def __init__(self, file_name):
        ...
        self.input_file = f"input/{self.file_name}.c"
        self.output_file =f"output/{self.file_name}.c"
        self.iter_file = f"../../LoopInvGen_V/output/{self.file_name}.c"
    ...
    def extract_loop_file(self,output_file_path):
        # 读取原始文件内容
        with open(output_file_path, 'r', encoding='utf-8') as infile:
            code = infile.read()

            # 寻找每个 循环 开头和结尾的地方
        def extract_loop(code):
            # 将代码拆分成单字符的列表
            code_list = list(code)
            ...
            for idx, match in enumerate(matches):
                ...
                for i in range(loop_start - 1, -1, -1):  # 从 loop_start - 1 开始，反向遍历
                    if code_list[i] == '/' and  code_list[i+1] == '*' and code_list[i+2] == '@':
                        at_index = i
                        break  # 找到第一个 '@'，跳出循环
                ...
                # 提取循环内容
                loop_content =  ''.join(code_list[at_index:end_index])
        ...
        loop_contents = extract_loop(code)
    ...
    def process_loop_file(self,input_file_path, output_file_path):
        ...
        def process_loop(code):
            ...
            for idx, match in enumerate(matches):
                ...
                # 提取循环内容
                loop_content =  ''.join(code_list[loop_start:end_index])
        ...
        processed_code = process_loop(code)[0]
        loop_contents = process_loop(code)[1]
        sorted_indices = process_loop(code)[2]
    ...
    def execute(self):
        ...
        self.loop_contents = self.extract_loop_file(self.output_file)
        print(self.loop_contents)
        print(self.loop_entries)
